...
static struct z_erofs_pcluster_slab pcluster_pool[] __read_mostly = {
	_PCLP(1), _PCLP(4), _PCLP(16), _PCLP(Z_EROFS_CLUSTER_MAX_PAGES)
};
...
static struct z_erofs_pcluster *z_erofs_alloc_pcluster(unsigned int nrpages)
{
	...
	for (i = 0; i < ARRAY_SIZE(pcluster_pool); ++i) {
		...
		mutex_init(&z_erofs_primarycollection(pcl)->lock);
		return pcl;
	}
				     ...
				     struct list_head *pagepool)
{
	...
	const unsigned int clusterpages = pcl->pclusterpages;
					     ...
					     struct erofs_map_blocks *map)
{
	...
	pcl = z_erofs_alloc_pcluster(max(1U, (unsigned int)(map->m_plen >> PAGE_SHIFT)));
	if (IS_ERR(pcl))
		return ERR_CAST(pcl);
				       ...
				       struct list_head *pagepool)
{
	...
	err = z_erofs_decompress(&(struct z_erofs_decompress_req) {
					...
					.inputsize = clusterpages * PAGE_SIZE,
				   ...
				   bool force_fg)
{
	...
	do {
		...
		clusterpages = pcl->pclusterpages;
