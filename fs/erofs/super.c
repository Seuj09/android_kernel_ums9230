...
static int erofs_read_superblock(struct super_block *sb)
{
	struct erofs_sb_info *sbi;
	struct page *page;
	struct erofs_super_block *dsb, dsb_copy;
	...
	memcpy(&dsb_copy, dsb, sizeof(dsb_copy));
	kunmap_atomic(data);
	put_page(page);

	/* parse on-disk compression configurations (may sleep) */
	if (erofs_sb_has_compr_cfgs(sbi))
		return erofs_load_compr_cfgs(sb, &dsb_copy);
	return z_erofs_load_lz4_config(sb, &dsb_copy, NULL, 0);
out:
	kunmap_atomic(data);
	put_page(page);
	return ret;
