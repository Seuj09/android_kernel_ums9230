...
if [ "$need_remount" = 1 ]; then
  ...
  grep -qs " $CG " /proc/mounts && { umount "$CG" 2>/dev/null || umount -l "$CG" 2>/dev/null || true; }
  mount -t cgroup2 -o rw,nosuid,nodev,noexec,relatime none "$CG" 2>/dev/null \
    || mount -t cgroup2 none "$CG" 2>/dev/null || { log "mount FAILED"; exit 0; }
fi
[ -f "$CG/cgroup.controllers" ] || exit 0
enable_dir() {
  _d=$1; mkdir -p "$_d" 2>/dev/null || true
  [ -f "$_d/cgroup.controllers" ] || return 0
  _avail=$(cat "$_d/cgroup.controllers"); _line=""
  for _c in cpuset cpu io memory pids; do
    case " $_avail " in *" $_c "*) _line="$_line +$_c" ;; esac
  done
  _line=$(echo "$_line" | sed 's/^ //'); [ -n "$_line" ] || return 0
  echo "$_line" >"$_d/cgroup.subtree_control" 2>/dev/null || true
}
enable_dir "$CG"
...
enable_dir "$CG/apps"; enable_dir "$CG/system"
log "done"; exit 0
