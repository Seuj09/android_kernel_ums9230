...
static ssize_t comp_algorithm_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return __comp_algorithm_show(dev_to_zram(dev), ZRAM_PRIMARY_COMP, buf);
}

static ssize_t comp_algorithm_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	int ret = __comp_algorithm_store(dev_to_zram(dev), ZRAM_PRIMARY_COMP, buf);
	return ret ? ret : len;
}
			   ...
			   u32 threshold, u32 prio, u32 prio_max)
{
	...
	unsigned long handle_old, handle_new = 0;
	unsigned int comp_len_old, comp_len_new = 0;
	unsigned int class_index_old, class_index_new = 0;
	u32 num_recomps = 0;
	void *src, *dst;
	int ret;
