...
{
	mm->lru_gen = kzalloc(sizeof(*mm->lru_gen), GFP_KERNEL);
	if (!mm->lru_gen)
		return -ENOMEM;
...
{
	/*
	 * mm->lru_gen is heap-allocated (see the comment on struct lru_gen_mm
	 * in mm_types.h) and can be NULL if the kzalloc() in lru_gen_init_mm()
	 * failed; degrade gracefully instead of crashing the hottest path in
	 * the scheduler.
	 */
	if (!mm->lru_gen)
		return;
...
void lru_gen_del_mm(struct mm_struct *mm)
{
	...
	if (!mm->lru_gen || list_empty(&mm->lru_gen->list))
		return;
			    ...
			    struct mm_struct **iter)
{
	...
	while (!mm && mm_state->head != &mm_list->fifo) {
		mm = list_entry(mm_state->head, struct lru_gen_mm, list)->mm;
...
static void lru_gen_age_node(struct pglist_data *pgdat, struct scan_control *sc)
{
	 ...
	 *
	 * No CONFIG_ANDROID_SIMPLE_LMK on this tree — always use the classic
	 * out_of_memory() path under oom_lock.
	 */
	if (!success) {
...
static bool drain_evictable(struct lruvec *lruvec)
{
	...
	for_each_gen_type_zone(gen, type, zone) {
		...
		while (!list_empty(head)) {
			...
			VM_BUG_ON(!success);
			/* lru_gen_del_page() already set PG_active correctly above */
			add_page_to_lru_list(page, lruvec, page_lru(page));
...
int kswapd_run(int nid)
{
	...
	ret = kfifo_alloc((struct kfifo *)pgdat->kcompress_fifo,
			...
			GFP_KERNEL);
	if (ret) {
		pr_err("%s: fail to kfifo_alloc for kcompressd\n", __func__);
